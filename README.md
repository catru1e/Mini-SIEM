...
The system collects Linux security events from demo log files, real Linux log files, process snapshots, and file activity monitoring. Events are normalized, enriched by the server, stored in SQLite, processed by detection and correlation logic, and displayed in a web dashboard.
...
6. Press d in the launcher.
7. Choose logs/bruteforce.txt.
8. Show failed login events in the dashboard.
9. Show generated alerts.
10. Press d again.
11. Choose logs/sudo_test.txt.
12. Show privilege escalation events.
13. Show system_event entries from syslog or kern.log.
14. Press s to open the full server log.
15. Press a to open the full agent log.
16. Press q to stop everything.
```
...
```text
collection -> normalization -> ingestion -> storage -> detection -> correlation -> dashboard
```
