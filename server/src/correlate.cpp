...
std::vector<json> CorrelationEngine::process_event(const json& event) {
  std::vector<json> alerts;
  const std::string event_type = safe_string(event, "event_type");

  if (event_type == "auth_success") {
    auto a = correlate_bruteforce_success(event);
...
json CorrelationEngine::correlate_bruteforce_success(const json& event) {
  ...
  if (failed_count < 5) return nullptr;
